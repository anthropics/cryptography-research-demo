 ...
 *
 * Build: gcc -O3 -march=native -I../common -o online_bench online_bench.c
...
static u64 bench_Bpub(const octx_t*O,const kstream_t*K,u64 N){
    u64 acc=0;
    for(u64 n=0;n<N;n++){
        const u8*k=K->keys[n];
        for(int s=0;s<256;s++){
            u64 h=dfj_published_s(O,k,(u8)s);
            acc^=h; acc+=ht_probe(h);
        }
    }
    return acc;
}
...
static stat_t runBpub(const octx_t*O,const kstream_t*K,u64 N,int reps,const char*name){
    double t[16];
    (void)bench_Bpub(O,K,N>>6);                           /* warm-up (untimed) */
    double r0=calib_ratio();
    double c0=cpu_ns();
    for(int r=0;r<reps;r++){
        u64 t0=rdtsc_();
        u64 acc=bench_Bpub(O,K,N);
        u64 t1=rdtsc_();
        SINK^=acc;
        t[r]=(double)(t1-t0)/(double)N;
    }
...
int main(int argc,char**argv){
    ...
    stat_t Bpub=runBpub(O,&KP,NP,reps,"DFJ-published plain no-exit");
    loadavg("end");
    ...
    printf("\n=== HEAD-TO-HEAD interleaved (per-round ratio, %d rounds; A chunk 2^%d, B chunk 2^%d, Bpub chunk 2^%d cands) ===\n",21,logNA-4,logNB-4,logNP-10);
    {
        ...
        for(int r=0;r<RR;r++){
            ...
            t0=rdtsc_(); SINK^=bench_Bpub(O,&KP,nP); t1=rdtsc_(); pB[r]=(double)(t1-t0)/nP;
