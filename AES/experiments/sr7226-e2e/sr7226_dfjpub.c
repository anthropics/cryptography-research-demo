...
/* ---- "DFJ as published" online variant -------------------------------
 * DFJ_PUB=1  : inside the 64-value u_5[0]=s loop the ciphertext peel of the
 *              delta-set (v[w] = T0[ct0[w]^k60]^T1[ct3[w]^k63]) is RECOMPUTED
 *              for every s (no sharing across the s loop), and the s loop does
 *              NOT early-exit on a Bloom hit.  Plain scalar.  Everything else
 *              (candidate enumeration, Bloom probes, key-schedule disposal)
 *              is identical to attack_dfj.
 * SPOT_STRIDE>0 : gate (b) mode.  No table, no key solve.  Every
 *              SPOT_STRIDE-th candidate, for ALL 64 values of s, compute the
 *              per-candidate multiset-hash stream both ways (shared peel =
 *              attack_dfj, recomputed peel = attack_dfj_pub) and count
 *              mismatches.                                                  */
static int DFJ_PUB=0;
static long SPOT_STRIDE=0;
static void attack_full(const char *bloom_path, int log2bits, unsigned seed_mk,
                        int self_only){
    srand(seed_mk);
    gf MK[4]={rnd6(),rnd6(),rnd6(),rnd6()}, RK[NROUNDS+1][4];
    key_schedule(MK,RK);
    if(!self_only && !SPOT_STRIDE) bloom_map(bloom_path,log2bits,0);
    long Tpairs=0,Tlk=0,Thit=0,Tks=0,Trawhit=0; int Tstruct=0;
    long Tspot=0,Tspotbad=0,Tspotcand=0;
  ...
  for(int str=0; str<16 && !recovered; str++){
    ...
    /* ---- Bloom source ---- */
    uint64_t SELF_FP[2]={0,0}, SELF_MS=0;
    if(self_only && SPOT_STRIDE){ /* spot gate needs no table entry */ }
    else if(self_only){
        ...
        { gf dtrue[Q]; for(int w=0;w<Q;w++) dtrue[w]=(gf)(s^x5[w]); SELF_MS=mset_hash(dtrue,Q); }
        printf("self_only: right pair P=(%d,%d)/(%d,%d)  s=%02x  fp=%016llx/%016llx  ms=%016llx\n",
               refP0,refP3,othP0,othP3,s,
               (unsigned long long)SELF_FP[0],(unsigned long long)SELF_FP[1],
               (unsigned long long)SELF_MS);
    }
    #define LOOKUP(h)    (self_only ? ((h)==SELF_FP[0]||(h)==SELF_FP[1]) : bloom_test(h))
    #define LOOKUP_MS(h) (self_only ? ((h)==SELF_MS) : bloom_test(h))
    ...
    long npairs=0, nlk=0, nhit=0, nks=0, nrawhit=0;
    long ncand=0, nspot=0, nspotbad=0, nspotcand=0;
    long spot_off = SPOT_STRIDE ? (rand()%SPOT_STRIDE) : 0;   /* random sampling phase */
            ...
            for(int ik3=0;ik3<nk3;ik3++){
                ...
                for(int k60=0;k60<Q;k60++){
                    ...
                    for(int ik63=0;ik63<nk63;ik63++){
                        ...
                        int hit_here=0;
                        if(DFJ_MODE && SPOT_STRIDE){
                            /* gate (b): spot-check hash-stream equality pub vs shared-peel */
                            if((ncand++ % SPOT_STRIDE)==spot_off){
                                nspotcand++;
                                gf v[Q];
                                for(int w=0;w<Q;w++) v[w]=(gf)(T0[ct0[w]^k60]^T1[ct3[w]^k63]);
                                for(int u5=0;u5<Q;u5++){
                                    /* shared-peel stream (attack_dfj) */
                                    gf a0=SINV[v[0]^u5];
                                    gf dd[Q]; for(int w=0;w<Q;w++) dd[w]=(gf)(a0^SINV[v[w]^u5]);
                                    uint64_t h_sh=mset_hash(dd,Q);
                                    /* recomputed-peel stream (attack_dfj_pub) */
                                    gf a0p=SINV[(gf)(T0[ct0[0]^k60]^T1[ct3[0]^k63])^u5];
                                    gf ddp[Q];
                                    for(int w=0;w<Q;w++)
                                        ddp[w]=(gf)(a0p^SINV[(gf)(T0[ct0[w]^k60]^T1[ct3[w]^k63])^u5]);
                                    uint64_t h_pb=mset_hash(ddp,Q);
                                    nspot++;
                                    if(h_sh!=h_pb) nspotbad++;
                                }
                            }
                            continue;
                        }
                        if(DFJ_MODE && DFJ_PUB){
                            /* DFJ as published: peel recomputed per s, no early exit */
                            for(int u5=0;u5<Q;u5++){
                                nlk++;
                                gf a0=SINV[(gf)(T0[ct0[0]^k60]^T1[ct3[0]^k63])^u5];
                                gf dd[Q];
                                for(int w=0;w<Q;w++)
                                    dd[w]=(gf)(a0^SINV[(gf)(T0[ct0[w]^k60]^T1[ct3[w]^k63])^u5]);
                                if(LOOKUP_MS(mset_hash(dd,Q))){ hit_here=1; nrawhit++; }
                            }
                            ...
                            for(int u5=0;u5<Q;u5++){
                                ...
                                gf a0=SINV[v[0]^u5];
                                gf dd[Q]; for(int w=0;w<Q;w++) dd[w]=(gf)(a0^SINV[v[w]^u5]);
                                if(LOOKUP_MS(mset_hash(dd,Q))){ hit_here=1; nrawhit++; break; }
                            }
                            if(!hit_here) continue;
                            nhit++;
                        } else {
    ...
    Tpairs+=npairs; Tlk+=nlk; Thit+=nhit; Tks+=nks; Trawhit+=nrawhit;
    Tspot+=nspot; Tspotbad+=nspotbad; Tspotcand+=nspotcand;
    fprintf(stderr,"  [struct %d] pairs=%ld lk=%ld hits=%ld rec=%d\n",
            str,npairs,nlk,nhit,recovered);
    if(SPOT_STRIDE){
        printf("\n-- gate (b): spot check, seed=%u  structure 0 --\n",seed_mk);
        printf("  candidates enumerated : %ld\n",ncand);
        printf("  candidates spot-checked: %ld (stride %ld)\n",nspotcand,SPOT_STRIDE);
        printf("  (candidate,s) hash comparisons (pub vs dfj): %ld\n",nspot);
        printf("  mismatches: %ld  => %s\n",nspotbad, nspotbad?"FAIL":"PASS");
        return;
    }
  } /* structures */

    double secs=wall()-t0;
    printf("\n-- %s --\n", DFJ_PUB? "attack_dfj_pub (DFJ as published: per-s peel, no early exit, plain scalar)"
                         : DFJ_MODE? "attack_dfj (DFJ, shared peel)" : "attack_full (χ, DDT-constrained, no u_5[0])");
    printf("  structures      : %d\n",Tstruct);
    printf("  candidate pairs : %ld\n",Tpairs);
    printf("  %s lookups       : %ld\n",DFJ_MODE?"ms":"χ",Tlk);
    printf("  bloom/self hits : %ld\n",Thit);
    if(DFJ_MODE) printf("  raw probe hits  : %ld\n",Trawhit);
...
int main(int argc, char**argv){
    ...
    if(!strcmp(mode,"attack")){
        ...
        build_mode(path,log2bits,wu_lo,wu_hi,thr);
    } else if(!strcmp(mode,"attack_dfj") || !strcmp(mode,"attack_dfj_pub")){
        /* ./prog attack_dfj[_pub] <bloomfile> <log2bits> seed=N [self] [spot=STRIDE] */
        DFJ_MODE=1;
        DFJ_PUB=!strcmp(mode,"attack_dfj_pub");
        const char *path=(argc>2)?argv[2]:"bloom_dfj.bin";
        int log2bits=(argc>3)?atoi(argv[3]):40;
        int self=0;
        for(int i=4;i<argc;i++){
            if(!strcmp(argv[i],"self")) self=1;
            if(!strncmp(argv[i],"spot=",5)) SPOT_STRIDE=atol(argv[i]+5);
        }
        if(SPOT_STRIDE) self=1;   /* spot gate needs no table */
